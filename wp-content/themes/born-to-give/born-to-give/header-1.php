...
<div class="site-header-wrapper">
    ...
    <header class="site-header">
        <div class="container">
            <div class="site-logo">
                ...
                <?php } else { ?>
                    <?php if (get_locale() == "ar") { ?>
                        <a href="<?php echo esc_url(home_url('/')); ?>" class="default-logo"><img
                                src="<?php echo get_template_directory_uri(); ?>/images/loogo.png?>" alt="<?php echo get_bloginfo('name'); ?>" style=" max-width: 171px; " ></a>
                        <?php } else {
                            ?>
                        <a href="<?php echo esc_url(home_url('/')); ?>" class="default-logo"><img
                                src="<?php echo esc_url($logo_url); ?>" alt="<?php echo get_bloginfo('name'); ?>"></a>
                        <?php } ?>

                    <a href="<?php echo esc_url(home_url('/')); ?>" class="default-retina-logo"><img
                            src="<?php echo esc_url($logo_retina_url); ?>" alt="<?php echo get_bloginfo('name'); ?>"
                            width="<?php echo esc_attr($retina_logo_width); ?>"
                            height="<?php echo esc_attr($retina_logo_height); ?>"></a>
                        <?php if (get_locale() == "ar") { ?>
                        <a href="<?php echo esc_url(home_url('/')); ?>" class="sticky-logo"><img
                                src="<?php echo get_template_directory_uri(); ?>/images/loogo.png?>"
                                alt="<?php echo get_bloginfo('name'); ?>" style="max-width:113px" ></a>
                        <?php } else {
                            ?>
                        <a href="<?php echo esc_url(home_url('/')); ?>" class="sticky-logo"><img
                                src="<?php echo esc_url($logo_sticky_url); ?>"
                                alt="<?php echo get_bloginfo('name'); ?>"></a>
                        <?php } ?>
                    <a href="<?php echo esc_url(home_url('/')); ?>" class="sticky-retina-logo"><img
                            src="<?php echo esc_url($logo_retina_sticky_url); ?>"
                            alt="<?php echo get_bloginfo('name'); ?>"
                            width="<?php echo esc_attr($sticky_retina_logo_width); ?>"
                            height="<?php echo esc_attr($sticky_retina_logo_height); ?>"></a>
                    <?php } ?>
            </div>
            <a href="#" class="visible-sm visible-xs" id="menu-toggle"><i class="fa fa-bars"></i></a>
            <?php if (isset($borntogive_options['header_info_1_text']) && $borntogive_options['header_info_1_text'] != '') { ?>
                <?php the_widget('qTranslateXWidget', array('type' => 'custom', 'hide-title' => true)); ?>

                <div class="header-info-col"><?php if ($borntogive_options['header_info_1_icon'] != '') { ?><i
                            class="fa <?php echo esc_attr($borntogive_options['header_info_1_icon']); ?>"></i><?php } ?><?php echo esc_attr($borntogive_options['header_info_1_text']); ?>
                </div><?php } ?>
            <?php
            if (isset($borntogive_options['enable-search']) && $borntogive_options['enable-search'] == 1) {
                imic_search_button_header();
            }
            ?>
            <?php
            if (isset($borntogive_options['enable-cart']) && $borntogive_options['enable-cart'] == 1) {
                echo imic_cart_button_header();
            }
            ?>
            <?php
            if (!empty($menu_locations['primary-menu'])) {
                wp_nav_menu(array('theme_location' => 'primary-menu', 'container' => '', 'items_wrap' => '<ul style="margin-top:25px" id="%1$s" class="sf-menu dd-menu pull-right">%3$s</ul>', 'walker' => new borntogive_mega_menu_walker));
            }
            ?>
            <?php
            if (get_locale() == "ar") {
                $url = '?page_id=1615&lang=ar';
                $name = 'تبرع';
            } else {
                $url = '?page_id=169';
                $name = 'Donate';
            ...
            ?>
        <!--    <div class="donate_button"><a href="/<?php echo $url; ?>">-->
