...
wp_localize_script('borntogive_event_register_validation', 'event_registration', array('url' => admin_url('admin-ajax.php'),'name'=>$invalid_name,'emails'=>$invalid_email, 'process'=>$process, 'tickets'=>$tickets_empty));
						...
						foreach($tickets_type as $tickets)
						{
							...
							$field_tickets_available = ($available_ticket>10)?10:$available_ticket;
								echo '<tr>';
									...
									{
										//Tickets Total Price
										echo ''.esc_attr__('Total: ', 'borntogive').$paypal_currency.' <span class="total-cost-event"></span></label>';
									}
									echo '</td>';
								echo '</tr>';
