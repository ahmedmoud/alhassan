...
/* These arguments are going to function like variables, allowing us to set new values in the front-end editor */
 ...
 return $output.
 do_shortcode('[campaigns number="'.$posts_per_page.'" orderby="'.$causes_orderby.'" creator="'.$causes_users.'" include_inactive="'.$causes_inactive.'" id="'.$causes_ids.'" exclude="'.$causes_exclude.'" category="'.$causes_terms.'" columns="'.$causes_grid_column.'-'.$causes_view.'-'.$causes_filters.'-'.$show_causes_excerpt.'-'.$show_causes_pagination.'-'.$causes_number.'"]').
 $output_close;
...
function borntogive_team_element_output( $atts, $content = null) {
	...
	{
		$team_output .= '<ul class="isotope-grid team-custom-grid">';	
	}
	...
	if($team_carousel!=1)
	{
		$team_output .= '<li class="grid-item team-custom-grid-item format-standard" style="width:'.$team_grid_column.'">
	...
	$team_output .= '<div class="grid-item-content">
                                	<h3 class="margin-5"><a href="'.get_permalink().'">'.get_the_title().'</a></h3>';
	if($position!='')
	{                             
	...
	if($team_details==1&&($team_excerpt != '' &&$team_excerpt != 0))
	{
		$team_output .= borntogive_excerpt($team_excerpt, $team_closing_text, $team_more_text);
	}
