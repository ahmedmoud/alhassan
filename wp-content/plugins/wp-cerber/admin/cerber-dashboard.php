...
function crb_admin_quick_nav( $context = '', $class = 'crb-button-tiny' ) {
	...
	if ( ! $context ) {
		$links[] = array( $base_url, __( 'View all', 'wp-cerber' ) );
	...
	if ( $context == 'users' ) {
		$links[] = array( $base_url . '&amp;filter_user=*', __( 'View all', 'wp-cerber' ) );
	}

	if ( $context != 'suspicious' ) {
		$links[] = array( cerber_activity_link( array( 1, 2 ) ), __( 'New users', 'wp-cerber' ) );
		$links[] = array( $base_url . '&amp;filter_set=2', __( 'Login issues', 'wp-cerber' ) );
	}
	...
	if ( $context != 'users' ) {
		...
		$links[] = array( $base_url . '&amp;filter_set=1', $txt );
	}

	if ( ! $context ) {
		$links[] = array( cerber_activity_link( array( 10, 11 ) ), __( 'IP blocked', 'wp-cerber' ) );
		$links[] = array( $base_url . '&amp;filter_user=*', __( "Users' activity", 'wp-cerber' ) );
		$links[] = array( $base_url . '&amp;filter_user=0', __( 'Non-authenticated', 'wp-cerber' ) );
		$links[] = array( $base_url . '&amp;filter_user=' . get_current_user_id(), __( 'My activity', 'wp-cerber' ) );
		$links[] = array( $base_url . '&amp;filter_ip=' . cerber_get_remote_ip(), __( 'My IP', 'wp-cerber' ) );
	}

	$ret = '';
	foreach ( $links as $link ) {
		$ret .= '<a class="' . $class . '" href="' . $link[0] . '">' . $link[1] . '</a> ';
	}
...
function cerber_show_activity( $args = array(), $echo = true ) {
	...
	if ( empty( $args['no_navi'] ) ) {
		...
		$ret = '<div><div class="crb-quick-nav">' . crb_admin_quick_nav() . '</div>' . $top_bar . $info . '</div>' . $ret;
	}
...
function cerber_ip_extra_view( $ip, $context = 'activity' ) {
	...
	$ret = '<div class="crb-extra-info crb-extra-ip">
		<table>
...
function cerber_user_extra_view( $user_id, $context = 'activity' ) {
	...
	//$ret .= '<div id="crb_the_summary">' . $summary . '</div><div style="float: left;" class="crb_async_content" data-ajax_route="user_activity_analytics" data-user_id="' . $user_id . '"></div>';
    $ret .= '<div id="crb_the_summary">' . $summary . '</div>';
	...
	return '<div class="crb-extra-info ' . $class . '" id="user-extra-info">' . $ret . '</div>';
...
add_filter( 'manage_users_custom_column', function ( $value, $column, $user_id ) {
	...
	switch ( $column ) {
		...
		case 'cbfl' :
			...
			if ( $failed ) {
				$act_link = cerber_activity_link( array( 7 ) ) . '&amp;filter_login=' . $u->user_email . '|' . $u->user_login;
				$ret = '<a href="' . $act_link . '">' . $failed . '</a>';
			}
			else {
				$ret = $failed;
			}
...
function cerber_show_dashboard() {
	...
	$nav_links = ( $logged ) ? crb_admin_quick_nav( 'users', '' ) : '';
	...
	$nav_links = ( $bad_logged ) ? crb_admin_quick_nav( 'suspicious', '' ) : '';
...
function cerber_show_traffic( $args = array(), $echo = true ) {
	...
	if ( ! $wp_cerber_remote ) {
		...
		if ( $rows ) {
			...
			//if ( $act_events = cerber_db_get_results( 'SELECT * FROM ' . CERBER_LOG_TABLE . ' WHERE session_id IN ("' . implode( '", "', array_keys( $rows ) ) . '" )', MYSQL_FETCH_OBJECT ) ) {
	...
	if ( empty( $args['no_navi'] ) ) {
		...
		$filters[] = array( '&amp;filter_set=1', __( 'Suspicious requests', 'wp-cerber' ) );
		$filters[] = array( '&amp;filter_http_code=399&filter_http_code_mode=GT', __( 'Errors', 'wp-cerber' ) );
		$filters[] = array( '&amp;filter_user=*', __( 'Users', 'wp-cerber' ) );
		$filters[] = array( '&amp;filter_user=0', __( 'Non-authenticated', 'wp-cerber' ) );
		$filters[] = array(
